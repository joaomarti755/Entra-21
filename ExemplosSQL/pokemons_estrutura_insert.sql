...
-- Ordenar em ordem crescente pelo ataque
...
SELECT nome, codigo, categoria, descricao, altura, peso, hp, ataque, defesa, especial_ataque, especial_defesa, velocidade, ROUND(imc, 2) -- ROUND(imc, 2) ultilizado para formatar o imc
	FROM pokemons;

UPDATE pokemons SET imc = (peso / (altura * altura)) FROM pokemons;

SELECT altura, peso, ROUND(imc, 2)
	FROM pokemons
	ORDER BY imc DESC;

ALTER TABLE pokemons ADD tamanho_nome INT;
-- Achando o menor nome
SELECT nome, tamanho_nome = LEN (nome)
	FROM pokemons
	ORDER BY LEN (nome) DESC;

-- Exibindo nome e descrição caso o nome possuir mais de 10 caracteres
SELECT nome, descricao
	FROM pokemons
	WHERE
		LEN (nome) > 10;

-- Descobrir o menor ataque
SELECT nome, categoria, ataque
	FROM pokemons
...
-- Descobrir a média do ataque
SELECT AVG(ataque)AS 'Média' FROM pokemons;

-- Descobrir a soma dos ataques
SELECT SUM(ataque)AS 'Soma ataque' FROM pokemons;

SELECT AVG(especial_ataque)AS 'Média Ataque especiais'
	FROM pokemons
	WHERE nome LIKE 'p%';